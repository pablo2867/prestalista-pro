// app/api/dashboard/route.ts - VERSIÓN CON CLIENTE COMPARTIDO
...
export async function GET() {
  try {
    // Capital
    const { data: capitalData } = await supabaseServer.from('capital_movements').select('balance_after, movement_type, amount').order('created_at', {ascending:false}).limit(1)
    const { data: allCapital } = await supabaseServer.from('capital_movements').select('movement_type, amount')
    ...
    // Distribuidores
    const { data: distributors } = await supabaseServer.from('distributors').select('id')
    
    // Prestatarios
    const { data: prestatarios } = await supabaseServer.from('prestatarios').select('estado')
    ...
    // Préstamos
    const { data: prestamos } = await supabaseServer.from('prestamos').select('estado, monto_principal, saldo_pendiente')
    ...
    // Leads
    const { data: leads } = await supabaseServer.from('leads').select('estado, monto_interes')
