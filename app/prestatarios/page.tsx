...
export default function PrestatariosPage() {
  ...
  const syncToSheets = async () => {
    ...
    try { await fetch(GOOGLE_SHEETS_WEBHOOK, { method: 'POST', mode: 'no-cors', keepalive: true, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tipo: 'Prestatario', datos: payload, fecha: new Date().toISOString() }) }); showToast('✅ Sincronizado', 'success') }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0b0f19', color: 'white', padding: '32px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: '16px', padding: '32px', marginBottom: '32px' }}>
        ...
        <button onClick={syncToSheets} disabled={syncing} style={{ padding: '12px 24px', backgroundColor: syncing ? '#6b7280' : '#10b981', color: 'white', border: 'none', borderRadius: '8px', cursor: syncing ? 'not-allowed' : 'pointer', fontWeight: '600' }}>{syncing ? '🔄 Sincronizando...' : '🔄 Sheets'}</button>
      </div>
