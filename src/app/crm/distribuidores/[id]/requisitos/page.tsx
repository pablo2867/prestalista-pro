...
import { 
  ...
  Eye, Trash2, AlertTriangle, FolderOpen, ArrowLeft,
  Shield, Loader2, UserCheck, Briefcase, Banknote, Award
} from 'lucide-react'

// Requisitos específicos para Distribuidores/Agentes
const REQUISITOS_DISTRIBUIDORES = [
  { id: 'ine', nombre: 'INE / Identificación Oficial', tipo: 'image', obligatorio: true, descripcion: 'Foto frontal y trasera', validable: false },
  { id: 'curp', nombre: 'CURP', tipo: 'text', obligatorio: true, descripcion: 'Clave Única de Registro de Población', validable: true },
  { id: 'rfc', nombre: 'RFC con Homoclave', tipo: 'pdf', obligatorio: true, descripcion: 'Documento oficial del SAT', validable: false },
  { id: 'comprobante_domicilio', nombre: 'Comprobante de Domicilio', tipo: 'pdf', obligatorio: true, descripcion: 'Recibo de luz, agua o teléfono (menos de 3 meses)', validable: false },
  { id: 'comprobante_bancario', nombre: 'Comprobante de Cuenta Bancaria', tipo: 'pdf', obligatorio: true, descripcion: 'Estado de cuenta o carta del banco (CLABE visible)', validable: false },
  { id: 'cedula_profesional', nombre: 'Cédula Profesional', tipo: 'pdf', obligatorio: false, descripcion: 'Cédula profesional (si aplica)', validable: false },
  { id: 'antecedentes_no_penales', nombre: 'Carta de Antecedentes No Penales', tipo: 'pdf', obligatorio: true, descripcion: 'Expedida en los últimos 6 meses', validable: false },
  { id: 'contrato_firmado', nombre: 'Contrato de Distribución Firmado', tipo: 'pdf', obligatorio: true, descripcion: 'Contrato firmado por ambas partes', validable: false },
  { id: 'constancia_capacitacion', nombre: 'Constancia de Capacitación', tipo: 'pdf', obligatorio: true, descripcion: 'Curso de productos y procesos completado', validable: false },
  { id: 'foto_perfil', nombre: 'Foto de Perfil Profesional', tipo: 'image', obligatorio: true, descripcion: 'Foto tipo credencial para sistema', validable: false },
  { id: 'referencia_comercial', nombre: 'Referencia Comercial', tipo: 'text', obligatorio: false, descripcion: 'Nombre y teléfono de referencia comercial', validable: false },
  { id: 'experiencia_ventas', nombre: 'Experiencia en Ventas', tipo: 'text', obligatorio: false, descripcion: 'Descripción de experiencia previa en ventas', validable: false }
]

export default function RequisitosDistribuidorPage() {
  ...
  useEffect(() => {
    setTimeout(() => {
      const requisitosConEstado = REQUISITOS_DISTRIBUIDORES.map(req => ({
        ...req,
        ...
        notas: '',
        valorTexto: req.id === 'curp' ? 'MELC850101HDFRNN09' : req.id === 'experiencia_ventas' ? '5 años en ventas financieras' : ''
      }))
  ...
  const downloadAllDocuments = async () => {
    ...
    alert(`📦 Descargando ${docs.length} documento(s) en ZIP...\n\nEn producción, esto generaría un archivo .zip con todos los documentos del distribuidor.`)
  }
      ...
      <main className="lg:ml-64 p-6 md:p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Requisitos - {distribuidorInfo.nombre}</h1>
                <p className="text-gray-400 text-sm">Documentación requerida para distribuidor/agente</p>
              </div>
            </div>
          </div>
          <button
        ...
        <div className="bg-[#1a1a25] rounded-2xl p-4 border border-[#2a2a35] mb-6">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="text-gray-400">📧 {distribuidorInfo.email}</span>
            <span className="text-gray-400">📱 {distribuidorInfo.telefono}</span>
            <span className="text-gray-400">💼 Comisión: {distribuidorInfo.comision}</span>
            <span className="text-green-400">✅ {distribuidorInfo.estado}</span>
          ...
          <div className="bg-red-900/20 border border-red-700/50 rounded-xl p-4 mb-6 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-red-400/80 text-xs mt-1">
                {pendientesCount} requisito(s) obligatorio(s) aún no han sido recibidos. El distribuidor no puede operar hasta completarlos.
              </p>
            </div>
          </div>
        )}

        {/* Resultado de Validación CURP */}
        {curpResult && (
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-3">
                        {req.id === 'contrato_firmado' ? (
                          <Briefcase className={`w-5 h-5 ${req.obligatorio ? 'text-red-400' : 'text-gray-500'}`} />
                        ) : req.id === 'comprobante_bancario' ? (
                          <Banknote className={`w-5 h-5 ${req.obligatorio ? 'text-red-400' : 'text-gray-500'}`} />
                        ) : req.id === 'constancia_capacitacion' ? (
                          <Award className={`w-5 h-5 ${req.obligatorio ? 'text-red-400' : 'text-gray-500'}`} />
                        ) : (
